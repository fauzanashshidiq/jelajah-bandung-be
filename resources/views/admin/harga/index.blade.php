...
<div class="max-w-4xl mx-auto bg-white p-6 rounded shadow mt-10">
  ...
  <a href="{{ route('harga.create', $paket->id_paket) }}" class="mb-4 inline-block bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">
    + Tambah Harga
  </a>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td class="border border-gray-300 px-4 py-2 text-center space-x-2">
          <a href="{{ route('harga.edit', $harga->id_harga) }}" class="bg-green-600 text-white px-3 py-1 rounded hover:bg-green-700">Edit</a>
          <form action="{{ route('harga.destroy', $harga->id_harga) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
            @csrf
            @method('DELETE')
  ...
  <div class="mt-6">
    <a href="{{ route('paket-wisata.index', $paket->id_paket) }}" class="text-gray-600 hover:underline">← Kembali ke Paket Wisata</a>
</div>
</div>
@endsection
